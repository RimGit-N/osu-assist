...
def title():
    ...
    System.Clear()
    System.Title("Osussist By: RimGit-N")
    Cursor.HideCursor()
...
def get_move(coord_list, cursor_coords, strength):
    ...
    for coord in coord_list:
        ...
        if closest_coord_distance is None:
    ...
    if closest_coord is not None:
        x_diff = int(round((closest_coord[0] - cursor_coords[0]) * strength))
        y_diff = int(round((closest_coord[1] - cursor_coords[1]) * strength))

        if np.sqrt((closest_coord[0] - cursor_coords[0])**2 + (closest_coord[1] - cursor_coords[1])**2) <= aim_assist["range"]:
            new_cursor_coords = (cursor_coords[0] + x_diff, cursor_coords[1] + y_diff)
            win32api.SetCursorPos(new_cursor_coords)
            print(f"Moving mouse to: {new_cursor_coords}")
        else:
            print(f"Target too far away: {closest_coord[0]}, {closest_coord[1]}")
